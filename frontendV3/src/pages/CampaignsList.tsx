  ...
  return (
    <div className="bg-primary-dark ring-1 ring-neutral-700 rounded-xl shadow-lg overflow-hidden transition-all duration-300 ease-in-out hover:shadow-xl hover:shadow-accent-dark/60 hover:-translate-y-1 hover:ring-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent h-full">
      <div className="p-6 flex flex-col h-full">
        <h2 className="text-xl font-bold text-neutral-50 truncate">{campaign.title}</h2>
        {campaign.creatorUsername && (
          <p className="text-neutral-400 text-sm italic mt-1">Creado por: {campaign.creatorUsername}</p>
        )}
        
        <p className="text-neutral-300 mt-2 line-clamp-2 flex-grow min-h-[40px]">{campaign.description}</p>
        ...
        <div className="mt-5">
          ...
          <div className="flex justify-between items-center mt-3 text-sm">
            <span className="font-semibold text-neutral-50">${campaign.amountRaised.toLocaleString()}</span>
            <span className="text-neutral-400">Meta: ${campaign.goal.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
...
const CampaignsList = () => {
  ...
  return (
    <div>
      <h1 className="text-4xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-accent-light to-accent mb-10">
        Campañas Activas
      </h1>
      {renderContent()}
    </div>
